...
from boto3_wasabi.utils import ServiceContext
from boto3_wasabi.docs.client import boto3_wasabiClientDocumenter
from boto3_wasabi.docs.resource import ResourceDocumenter
from boto3_wasabi.docs.resource import ServiceResourceDocumenter


class ServiceDocumenter(BaseServiceDocumenter):
    # The path used to find examples
    EXAMPLE_PATH = os.path.join(os.path.dirname(boto3_wasabi.__file__), 'examples')

    def __init__(self, service_name, session):
        self._service_name = service_name
        self._boto3_wasabi_session = session
        ...
        self._client = self._boto3_wasabi_session.client(service_name)
        self._service_resource = None
        if self._service_name in self._boto3_wasabi_session.get_available_resources():
            self._service_resource = self._boto3_wasabi_session.resource(service_name)
        self.sections = [
            'title',
            'table-of-contents',
            'client',
    ...
    def client_api(self, section):
        ...
        boto3_wasabiClientDocumenter(self._client, examples).document_client(section)
    ...
    def _document_resources(self, section):
        ...
        for resource_name in json_resource_model['resources']:
            ...
            resource_cls = self._boto3_wasabi_session.resource_factory.\
