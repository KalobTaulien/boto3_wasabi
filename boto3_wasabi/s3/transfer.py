...
.. code-block:: python
    ...
    client = boto3_wasabi.client('s3', 'us-west-2')
...
.. code-block:: python
    ...
    transfer = S3Transfer(boto3_wasabi.client('s3', 'us-west-2'))
...
from boto3_wasabi.exceptions import RetriesExceededError, S3UploadFailedError
...
def create_transfer_manager(client, config, osutil=None):
    ...
    :type client: boto3_wasabi.client
    :param client: The S3 client to use

    :type config: boto3_wasabi.s3.transfer.TransferConfig
...
class S3Transfer(object):
    ...
    def __init__(self, client=None, config=None, osutil=None, manager=None):
        if not client and not manager:
            raise ValueError(
                'Either a boto3_wasabi.Client or s3transfer.manager.TransferManager '
                'must be provided'
            )
    ...
    def download_file(self, bucket, key, filename, extra_args=None,
        ...
        # exceeded we need to throw the same error from boto3_wasabi instead of
        # s3transfer's built in RetriesExceededError as current users are
        # catching the boto3_wasabi one instead of the s3transfer exception to do
        # their own retries.
