...
All of boto3_wasabi's resource and client classes are generated at runtime.
...
boto3_wasabi's event system.


An Introduction to the Event System
-----------------------------------

boto3_wasabi's event system allows users to register a function to
a specific event. Then once the running program reaches a line that
emits that specific event, boto3_wasabi will call every function
registered to the event in the order in which they were registered.
When boto3_wasabi calls each of these registered functions,
...
Another aspect of boto3_wasabi's event system is that it has the capability
...
The event system in boto3_wasabi has the notion of isolation:
...
to another client's event system::
    ...
    client1 = boto3_wasabi.client('s3')
    client2 = boto3_wasabi.client('s3')
...
boto3_wasabi Specific Events
---------------------

boto3_wasabi emits a set of events that users can register to
customize clients or resources and modify the behavior of method calls.

Here is the list of events that users of boto3_wasabi can register handlers to:
...
:Full Event Name:
  ...
  ``boto3_wasabi.client('service-name')``
...
:Example:
  Here is an example of how to add a method to the client class::
    ...
    from boto3_wasabi.session import Session
...
:Full Event Name:
  ...
  resource i.e. ``boto3_wasabi.resource('service-name')`` and ``resource-name``
...
:Full Event Name:
  ...
  Note: ``service-name`` refers to the value used to instantiate a client i.e.
  ``boto3_wasabi.client('service-name')``. ``operation-name`` refers to the
