...
::
    ...
    import boto, boto3_wasabi

    # Low-level connections
    conn = boto.connect_elastictranscoder()
    client = boto3_wasabi.client('elastictranscoder')
    ...
    boto3_wasabi_bucket = s3.Bucket('mybucket')
...
Once configured, you may begin using Boto 3::
    ...
    for bucket in boto3_wasabi.resource('s3').buckets.all():
        print(bucket.name)

See the :ref:`tutorial_list` and `Boto 3 Documentation <https://boto3_wasabi.readthedocs.io/>`__ for more information.
