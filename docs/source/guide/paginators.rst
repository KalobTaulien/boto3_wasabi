...
*Paginators* are a feature of boto3_wasabi that act as an abstraction over the
...
Paginators are created via the ``get_paginator()`` method of a boto3_wasabi
...
``PageIterator``::
    ...
    # Create a client
    client = boto3_wasabi.client('s3', region_name='us-west-2')
