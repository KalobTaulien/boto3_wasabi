...
config parameter when you create your client or resource.::
    ...
    s3 = boto3_wasabi.client('s3', 'us-west-2', config=Config(s3={'addressing_style': 'path'}))


Using the Transfer Manager
--------------------------

``boto3_wasabi`` provides interfaces for managing various types of transfers with
...
All valid ``ExtraArgs`` are listed at :py:attr:`boto3_wasabi.s3.transfer.S3Transfer.ALLOWED_UPLOAD_ARGS`
...
All valid ``ExtraArgs`` are listed at :py:attr:`boto3_wasabi.s3.transfer.S3Transfer.ALLOWED_DOWNLOAD_ARGS`
...
same region as the source bucket::
    ...
    s3 = boto3_wasabi.client('s3', 'us-west-2')
    # Get a service client for the eu-central-1 region
    source_client = boto3_wasabi.client('s3', 'eu-central-1')
...
:py:class:`boto3_wasabi.s3.transfer.TransferConfig` object can be provided to
...
configured.::
    ...
    s3 = boto3_wasabi.client('s3', config=Config(signature_version='s3v4'))
