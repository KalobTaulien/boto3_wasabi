...
from boto3_wasabi.s3.transfer import TransferConfig


class TestS3MethodInjection(unittest.TestCase):
    def test_transfer_methods_injected_to_client(self):
        session = boto3_wasabi.session.Session(region_name='us-west-2')
    ...
    def test_transfer_methods_injected_to_bucket(self):
        bucket = boto3_wasabi.resource('s3').Bucket('my_bucket')
    ...
    def test_transfer_methods_injected_to_object(self):
        obj = boto3_wasabi.resource('s3').Object('my_bucket', 'my_key')
...
class BaseTransferTest(unittest.TestCase):
    def setUp(self):
        self.session = boto3_wasabi.session.Session(
