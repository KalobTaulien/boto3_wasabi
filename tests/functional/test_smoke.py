...
def test_api_versions_synced_with_botocore():
    ...
    boto3_wasabi_session = create_session()
    for service_name in boto3_wasabi_session.get_available_resources():
        yield (_assert_same_api_versions, service_name,
               botocore_session, boto3_wasabi_session)


def _assert_same_api_versions(service_name, botocore_session, boto3_wasabi_session):
    resource = boto3_wasabi_session.resource(service_name)
    boto3_wasabi_api_version = resource.meta.client.meta.service_model.api_version
    client = botocore_session.create_client(service_name,
                                            region_name='us-east-1',
                                            aws_access_key_id='foo',
                                            aws_secret_access_key='bar')
    botocore_api_version = client.meta.service_model.api_version
    if botocore_api_version != boto3_wasabi_api_version:
        raise AssertionError(
            "Different latest API versions found for %s: "
            "%s (botocore), %s (boto3_wasabi)\n" % (service_name,
                                             botocore_api_version,
                                             boto3_wasabi_api_version))
