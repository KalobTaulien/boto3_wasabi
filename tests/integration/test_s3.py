...
import boto3_wasabi.s3.transfer


urlopen = six.moves.urllib.request.urlopen
LOG = logging.getLogger('boto3_wasabi.tests.integration')
...
def random_bucket_name(prefix='boto3_wasabi-transfer', num_chars=10):
...
def clear_out_bucket(bucket, region, delete_bucket=False):
    s3 = boto3_wasabi.client('s3', region_name=region)
...
class TestS3Resource(unittest.TestCase):
    def setUp(self):
        ...
        self.session = boto3_wasabi.session.Session(region_name=self.region)
...
class TestS3Transfers(unittest.TestCase):
    """Tests for the high level boto3_wasabi.s3.transfer module."""
    def setUp(self):
        self.region = _DEFAULT_REGION
        self.bucket_name = _SHARED_BUCKET
        clear_out_bucket(self.bucket_name, self.region)
    ...
    def create_s3_transfer(self, config=None):
        return boto3_wasabi.s3.transfer.S3Transfer(self.client,
                                            config=config)
